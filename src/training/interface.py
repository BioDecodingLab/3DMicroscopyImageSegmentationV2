...
def interface(
    ...
    if augmentation not in AVAILABLE_AUGMENTATIONS:
        raise typer.BadParameter(
            f"Unknown augmentation '{augmentation}'. Available: {AVAILABLE_AUGMENTATIONS}"
        )
    ...
    # Build per-run intensity params so repeated calls don't leak state into the module-level dict
    intensity_params = dict(INTENSITY_PARAMS)
    if psf_path and psf_path.exists():
        intensity_params["use_psf"] = True
        intensity_params["psf_path"] = str(psf_path)
        logger.info(f"PSF file activated: {psf_path}")
    else:
        intensity_params["use_psf"] = False
